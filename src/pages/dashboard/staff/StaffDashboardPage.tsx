        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="divide-y divide-gray-100">
                    {[
                        { id: 1, title: 'Urgent Request: Room 204', time: '10 mins ago', urgent: true },
                        { id: 2, title: 'Routine Check: Lobby Area', time: '45 mins ago', urgent: false },
                        { id: 3, title: 'Assist Guest: Poolside', time: '1 hour ago', urgent: false }
                    ].map((task) => (
                        <div key={task.id} className="p-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <div className="flex items-start gap-3">
                                <div className={`mt-1 w-2 h-2 rounded-full ${task.urgent ? 'bg-red-500' : 'bg-emerald-500'}`}></div>
                                <div>
                                    <p className="text-sm font-medium text-gray-900">
                                        {task.title}
                                    </p>
                                    <p className="text-xs text-gray-500">Assigned {task.time}</p>
                                </div>
                            </div>
