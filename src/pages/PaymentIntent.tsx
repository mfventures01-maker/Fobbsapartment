    ...
    useEffect(() => {
        ...
        const fetchOrder = async () => {
            try {
                ...
                if (data.status === 'paid') { setSuccess(true); setOrder(data); return; }
                if (data.status !== 'open') throw new Error('Order already processed');
        ...
        // ✅ PERMITTED: realtime subscription (non-table-query, event listener only)
        const channel = supabase
            .channel(`order-${orderId}`)
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders', filter: `id=eq.${orderId}` }, (payload) => {
                if (payload.new.status === 'paid') setSuccess(true);
            })
            .subscribe();
        ...
        return (
            <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-6 text-center">
                <div className="bg-red-500/10 border border-red-500/20 p-8 rounded-3xl max-w-sm w-full">
                    ...
                    <button onClick={() => window.location.href = '/'} className="w-full py-4 bg-slate-800 hover:bg-slate-700 rounded-2xl text-white font-bold transition-all">Return Home</button>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-[#0f172a] text-white selection:bg-blue-500/30 font-inter">
            <div className="max-w-md mx-auto min-h-screen flex flex-col relative px-6 py-12">
                ...
                <div className="text-center space-y-2 mb-10">
                    <h1 className="text-4xl font-black tracking-tight">₦{safeNumber(order.total)}</h1>
                ...
                <div className="flex-1">
                    {success ? (
                        <div className="space-y-6 animate-in fade-in zoom-in duration-500">
                            ...
                            <p className="text-center text-slate-500 text-sm italic py-4">This page will automatically update once payment is confirmed.</p>
                        </div>
                    ) : (
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid gap-3">
                                    {paymentOptions.map((option) => (
                                        <button key={option.id} onClick={() => handleSelect(option.id)} disabled={loading}
                                            className={`relative w-full p-5 rounded-2xl border-2 text-left transition-all duration-300 group ${paymentMethod === option.id ? 'border-blue-500 bg-blue-500/10 shadow-[0_0_20px_rgba(59,130,246,0.15)]' : 'border-slate-800 bg-slate-800/40 hover:border-slate-700 hover:bg-slate-800/60'}`}>
                                            <div className="flex items-center gap-4">
                                                <div className={`p-3 rounded-xl transition-all duration-300 ${paymentMethod === option.id ? 'bg-blue-500 text-white shadow-lg' : 'bg-slate-800 text-slate-400 group-hover:text-slate-200'}`}>{option.icon}</div>
                                                <div>
                                                    <p className={`font-bold text-lg ${paymentMethod === option.id ? 'text-white' : 'text-slate-300'}`}>{option.label}</p>
                                                    <p className="text-xs text-slate-500 font-medium">{option.description}</p>
                                                </div>
                                            </div>
                                            {paymentMethod === option.id && (<div className="absolute right-6 top-1/2 -translate-y-1/2"><div className="w-2 h-2 bg-blue-500 rounded-full shadow-[0_0_10px_#3b82f6] animate-pulse" /></div>)}
                                        </button>
                                    ))}
                                </div>
                            </div>
                            <button onClick={handleConfirm} disabled={!paymentMethod || loading}
                                className={`w-full py-5 rounded-2xl font-black text-lg tracking-wider transition-all duration-300 shadow-2xl active:scale-[0.98] ${!paymentMethod || loading ? 'bg-slate-800 text-slate-600 cursor-not-allowed border border-white/5' : 'bg-blue-600 hover:bg-blue-500 text-white shadow-blue-900/40'}`}>
                                {loading ? <div className="flex items-center justify-center gap-3"><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /><span>INITIATING...</span></div> : 'CONFIRM INTENT'}
                            </button>
                        </div>
                    )}
                </div>
                ...
                <div className="mt-auto pt-12 text-center">
                    <p className="text-[10px] font-bold text-slate-700 uppercase tracking-[0.3em]">Powered by CARSS Fintech</p>
                </div>
            </div>
        </div>
    );
