...
async function run() {
    ...
    // Try to update CEO's membership
    const ceoUid = '4731e41e-952b-4c20-b5b1-3ca39170d7b6';
    console.log("Updating CEO membership as SA...");
    const updateRes = await fetch(`${supabaseUrl}/rest/v1/business_memberships?user_id=eq.${ceoUid}`, {
        method: 'PATCH',
        headers: {
            'Content-Type': 'application/json',
            'apikey': supabaseKey,
