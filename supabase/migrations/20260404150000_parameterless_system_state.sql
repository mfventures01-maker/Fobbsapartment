-- 🛡️ CARSS FINAL FORENSIC SEAL: PARAMETERLESS SSOT PROVIDER
-- Purpose: Eliminate business_id passing for terminal state.
-- All operations resolve from internal identity spine (auth.uid()).
...
BEGIN
    ...
    IF v_ctx.v_business_id IS NULL THEN RAISE EXCEPTION 'Business context missing'; END IF;

    -- B. Order Aggregates
    SELECT COUNT(*) INTO v_open_orders_count
    FROM public.orders
    WHERE org_id = v_ctx.v_business_id AND location_id = v_ctx.v_branch_id AND status = 'open';

    SELECT COUNT(*) INTO v_pending_payment_count
    FROM public.orders
    WHERE org_id = v_ctx.v_business_id AND location_id = v_ctx.v_branch_id AND status = 'pending_payment';

    SELECT COUNT(*) INTO v_orders_today_count
    FROM public.orders
    WHERE org_id = v_ctx.v_business_id AND location_id = v_ctx.v_branch_id AND created_at >= CURRENT_DATE;

    -- C. Revenue Intelligence
    SELECT COALESCE(SUM(amount), 0) INTO v_revenue_today
    FROM public.transactions
    WHERE business_id = v_ctx.v_business_id AND branch_id = v_ctx.v_branch_id AND created_at >= CURRENT_DATE AND status IN ('verified', 'completed');

    SELECT COALESCE(SUM(amount), 0) INTO v_revenue_hour
    FROM public.transactions
    WHERE business_id = v_ctx.v_business_id AND branch_id = v_ctx.v_branch_id AND created_at >= NOW() - INTERVAL '1 hour' AND status IN ('verified', 'completed');
    ...
    WHERE business_id = v_ctx.v_business_id AND branch_id = v_ctx.v_branch_id AND status = 'pending';
    ...
    RETURN jsonb_build_object(
        'business_id', v_ctx.v_business_id,
        'branch_id', v_ctx.v_branch_id,
        ...
        'recent_transactions', (
            SELECT COALESCE(jsonb_agg(tx), '[]'::jsonb) FROM (
                ...
                FROM public.transactions
                WHERE business_id = v_ctx.v_business_id AND branch_id = v_ctx.v_branch_id
...
GRANT EXECUTE ON FUNCTION public.get_system_state() TO authenticated;
