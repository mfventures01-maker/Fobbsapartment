-- CARSS TERMINAL DISCOVERY UPGRADE
-- AIM: Implement device tracking and automated session expiration.

BEGIN;

-- 1. Add Device Info Column
ALTER TABLE public.terminal_sessions 
ADD COLUMN IF NOT EXISTS device_info TEXT;

-- 2. Automated Session Expiration Logic
-- This function marks any terminal that hasn't sent a heartbeat in 3 minutes as 'inactive'.
CREATE OR REPLACE FUNCTION public.expire_inactive_terminals()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- 3. Upgrade get_system_state to include detailed Terminal Radar data
...
BEGIN
    ...
    -- Prune dead terminals before state snapshot
    PERFORM public.expire_inactive_terminals();
    ...
    SELECT COALESCE(jsonb_agg(terminals), '[]'::jsonb) INTO v_terminal_list
    FROM (
        SELECT 
            ts.id,
            ts.terminal_type,
            ts.status,
            ts.device_info,
            ts.created_at as connection_time,
            ts.last_seen,
            sp.full_name as staff_name,
            sp.department
        FROM public.terminal_sessions ts
        JOIN public.staff_profiles sp ON ts.staff_id = sp.id
        WHERE ts.business_id = p_business_id
        AND (p_location_id IS NULL OR ts.branch_id = p_location_id)
    ...
    RETURN jsonb_build_object(
        ...
        'active_terminal_list', v_terminal_list,
